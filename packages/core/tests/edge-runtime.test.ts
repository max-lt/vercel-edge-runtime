...
describe('Using `instanceof`', () => {
  ...
  it('does not alter instanceof for literals and objects', async () => {
    ...
    expect(new EdgeRuntime().evaluate('new Array() instanceof Array')).toBe(
      true
    )
    expect(new EdgeRuntime().evaluate('/^hello$/gi instanceof RegExp')).toBe(
      true
    )
    expect(
      new EdgeRuntime().evaluate(
        'new RegExp("^hello$", "gi") instanceof RegExp'
      )
    ).toBe(true)
    expect(
      new EdgeRuntime().evaluate('({ foo: "bar" }) instanceof Object')
    ).toBe(true)
    expect(
      new EdgeRuntime().evaluate(
